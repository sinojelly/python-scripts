...
class Blog(object):
    ...
    def method_signature(self, method):
        """Call systen.methodSignature on server.

        Returns:
            Method signature.
        """
        try:
            return self.server.system.methodSignature(method)
        except xmlrpc.client.Fault as fault:
            raise BlogError(fault.faultString)

    def method_help(self, method):
        """Call systen.methodHelp on server.
    ...
    def get_capabilities(self):
        """Call systen.getCapabilities on server.

        Returns:
            System capabilities.
        """
        try:
            return self.server.system.getCapabilities()
        except xmlrpc.client.Fault as fault:
            raise BlogError(fault.faultString)
    ...
    def is_method_available(self, methodname):
        """Returns if a method is supported by the XML-RPC server (jelly: according to self.methods init in __init__)"""
